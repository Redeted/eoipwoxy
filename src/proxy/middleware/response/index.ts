...
async function handleGoogleAIRateLimitError(
  ...
  switch (status) {
    case "RESOURCE_EXHAUSTED": {
      // 429 from Google (including quota exhaustion) is temporary and resets on
      // its own, so never permanently disable a key here. Fall through to the
      // cooldown + retry handling below so the proxy keeps retrying.
/*
